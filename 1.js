...
class TranslatorsEngine {
  translateToMorse(text, dictionary) {
    return text
      ...
      .flatMap((e) =>
        [...e].map((el) =>
          Object.keys(dictionary).find((key) => dictionary[key] === el)
        )
      )
      .join(" ");
  }
...
const toMorseTranslator = new Translator(
  "hello world",
  translatorsEngine.translateToMorse,
  dictionary
);
console.log(toMorseTranslator.translate());

const toEnglishTranslator = new Translator(
  ".... . .-.. .-.. --- // .-- --- .-. .-.. -..",
  translatorsEngine.translateToEnglish,
  dictionary
);
